...
#define LOAD_FUNCTION(lib, name, ref) {ref=(__typeof__(ref))dlsym(lib, name); CHECK_DL_ERROR(ref, "Error getting entry point for " name);}
...
void AudioInputALSA::EnumerateDevices(std::vector<AudioInputDevice>& devs){
	...
	_snd_device_name_hint=(__typeof__(_snd_device_name_hint))dlsym(lib, "snd_device_name_hint");
	_snd_device_name_get_hint=(__typeof__(_snd_device_name_get_hint))dlsym(lib, "snd_device_name_get_hint");
	_snd_device_name_free_hint=(__typeof__(_snd_device_name_free_hint))dlsym(lib, "snd_device_name_free_hint");
